...
async def turnRight(angle, speed):
    motion_sensor.reset_yaw(0)
    while motion_sensor.tilt_angles()[0] >= angle * -10:
        motor.set_duty_cycle(port.A, speed)
        print(motion_sensor.tilt_angles()[0])
        await runloop.sleep_ms(100)
    motor.stop(port.A)

async def turnLeft(angle, speed):
    motion_sensor.reset_yaw(0)
    while motion_sensor.tilt_angles()[0] <= angle * 10:
        motor.set_duty_cycle(port.B, speed)
        print(motion_sensor.tilt_angles()[0])
        await runloop.sleep_ms(100)
    motor.stop(port.B)
